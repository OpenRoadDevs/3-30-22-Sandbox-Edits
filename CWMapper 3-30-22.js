 ...
 function (https, http, xml, encode, file, compress, search, record, format, CargoWiseLib) {
     ...
     function getPaymentXML(rec_type, rec_id) {
         ...
         try {
             ...
             log.debug('red id', rec_id);

             if (!rec) throw new Error('Couldn\'t load record'); 


            if (rec) {
                var lineObjects = rec.findSublistLineWithValue({ sublistId: 'apply', fieldId: 'apply', value: true  });
                var lineFields = rec.getSublistFields({ sublistId: 'apply' });
                var lineApply = rec.getSublistValue({ sublistId: 'apply', fieldId: 'apply', line: 1 });
                var lineRefNum = rec.getSublistValue({ sublistId: 'apply', fieldId: 'refnum', line: 1 });
            }; 
            log.debug('lineObjects', lineObjects);
            log.debug('lineFields', lineFields);
            log.debug('lineApply', lineApply);
            log.debug('lineRefNum', lineRefNum);
             ...
             var sch = search.create({
                 ...
                 filters: [ 
                    ...
                    //{name: 'tranid', operator: 'is', value: [rec_id]},
                 ],
                 columns: [
                    'externalid',
            ...
            if (rec) {
               var lineObjects = rec.getCurrentSublistIndex({ sublistId: 'apply', fieldId: 'apply', value: true  });
               var lineFields = rec.getSublistFields({ sublistId: 'apply' });
                var lineApply = rec.getSublistValue({ sublistId: 'apply', fieldId: 'apply', line: i });
                var lineRefNum = rec.getSublistValue({ sublistId: 'apply', fieldId: 'refnum', line: i });
           };

           if (lineObjects) {
            //    var indexLineObject = rec.getSublistValue({ sublistId: 'apply', fieldId: 'refnum', line: i });
            //    var indexLineTotal = rec.getSublistValue({ sublistId: 'apply', fieldId: 'total', line: i });
            //    var indexLineRefNum = rec.getSublistValue({ sublistId: 'apply', fieldId: 'refnum', line: i });
               var indexLineInternalId = rec.getSublistValue({ sublistId: 'apply', fieldId: 'internalid', line: i });
               };


                var recordFinder = record.load({ type: record.Type.VENDOR_BILL, id: indexLineInternalId, isDynamic: true });
              ...
              log.debug('info.jobkey', info.jobkey);
             ...
             log.debug('info.internalID', info.internalID);
             ...
             if (rec_type == record.Type.VENDOR_PAYMENT) {
                 ...
                 if (accttype == 'Bank') {
                     paymentInfo = {
                         type: 'Cash',
                         payment_type: 'CSH',
                         check_book_code: invoiceInfo.getValue('tranid'),//was rec.getValue
                         payment_ref: 'CSH',
             ...
             xml += ('                                <Key>' + info.JobKey + '</Key>');//was transkey (according to documentation, should be read as AP INV tran#)
             xml += ('                            </LinkedTransactionID>');
             xml += ('                        </LinkedTransactionIDCollection>');
             xml += ('                        <OSPaidAmount>' + (rec_type == record.Type.VENDOR_PAYMENT ? -1 * info.total : info.total) + '</OSPaidAmount>');// values were just total
             xml += ('                        <OrganizationAddress>');
             xml += ('                            <AddressType>' + 'OFC' +'</AddressType>');
             xml += ('                            <OrganizationCode>' + (info.entity_code) + '</OrganizationCode>');
             xml += ('                        </OrganizationAddress>');
             ...
             log.debug('this is the info.entity_code', info.entity_code);
             log.debug('current value passing through to request', xml);
           
                var response = CargoWiseLib.request(xml, true);
                log.debug('the response with differing values', response);
            }
